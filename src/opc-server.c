// This copy of opc-server.c is from Yona Appletree's fork of LEDscape.  It
// contains several fixes needed for use with Illumicone.
...
void* render_thread(void* unused_data)
{
	...
	for(;;) {
		...
		if (g_runtime_state.leds == NULL) {
			...
			pthread_mutex_unlock(&g_runtime_state.mutex);
            // We don't want to wait for minutes before seeing messages
            // in the system log when running as a service.
            fflush(stdout);
			usleep(1e6 /* 1s */);
			continue;
		}
